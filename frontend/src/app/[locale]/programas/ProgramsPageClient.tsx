...
export default function ProgramsPageClient({ programs }: { programs: StrapiProgram[] }) {
  const t = useTranslations('programs');

  const stats = [
    { value: String(programs.length || 13), label: t('stats.programs') },
        ...
        <div className="mx-auto max-w-7xl px-4 lg:px-8">
          ...
          <StaggerContainer scaleUp staggerDelay={0.06} className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
            {programs.map((program) => {
              ...
              const color = program.color ?? DEFAULT_COLOR;
              const imgSrc = getStrapiMedia(program.coverImage?.url ?? null) ?? LOCAL_IMAGES[program.slug] ?? '/images/news/festival-arte.webp';
