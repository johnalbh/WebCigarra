...
/* ---------- icon map (Strapi icon name → component) ---------- */
...
function TimelineNode({
  ...
  item: StrapiTimelineItem;
  index: number;
}) {
  const isEven = index % 2 === 0;
  const Icon = (item.icon && iconMap[item.icon]) || HiStar;
    ...
    <div className="relative mb-12 last:mb-0">
      ...
      >
        ...
        <div className="absolute left-8 z-10 -translate-x-1/2 md:left-1/2">
          ...
          >
            <div
              className={`flex h-10 w-10 items-center justify-center rounded-full bg-gradient-to-br ${color} shadow-sm`}
            >
        ...
        >
          <ScrollReveal direction={isEven ? 'left' : 'right'}>
            <div className="rounded-xl border border-gray-100 bg-white p-6 transition-colors duration-300 hover:border-gray-200">
              {/* Year badge */}
              <span
                className={`mb-3 inline-flex items-center gap-1.5 rounded-full bg-gradient-to-r ${color} px-4 py-1.5 text-sm font-bold text-white`}
              >
                {item.year}
              </span>
                ...
                <ul className="mt-2 space-y-2">
                  {item.items.map((text, i) => (
                    <li key={i} className="flex items-start gap-2 text-sm leading-relaxed text-gray-600">
                      <span className={`mt-1.5 h-1.5 w-1.5 shrink-0 rounded-full bg-gradient-to-br ${color}`} />
                      {text}
                    </li>
                  ))}
                </ul>
